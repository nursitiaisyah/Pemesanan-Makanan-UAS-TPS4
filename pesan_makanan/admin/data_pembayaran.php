                ...
                <tbody id="transaksiTable">
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td>
                                    <button class='btn btn-success bayar' data-id='" . $row['id'] . "'>Bayar</button>
                                    <button class='btn btn-primary cetak' data-id='" . $row['id'] . "'>Cetak</button>
                                  </td>";
                            echo "</tr>";
                            $no++;
                        }
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.cetak').on('click', function () {
                var id = $(this).data('id');
                var nomorPesanan = $(this).closest('tr').find('td:nth-child(2)').text();
                var tanggalTransaksi = $(this).closest('tr').find('td:nth-child(3)').text();
                var namaPelanggan = $(this).closest('tr').find('td:nth-child(4)').text();
                var namaMenu = $(this).closest('tr').find('td:nth-child(5)').text();
                var totalHarga = $(this).closest('tr').find('td:nth-child(6)').text();
                var jumlahBayar = $(this).closest('tr').find('input.jumlah-bayar').val();
                var kembalian = $(this).closest('tr').find('td:nth-child(8)').text();
                var status = $(this).closest('tr').find('td:nth-child(9)').text();

                var printWindow = window.open('', '', 'height=400,width=600');
                printWindow.document.write('<html><head><title>Cetak Transaksi</title>');
                printWindow.document.write('</head><body>');
                printWindow.document.write('<h2>Detail Transaksi</h2>');
                printWindow.document.write('<p>No. Pesanan: ' + nomorPesanan + '</p>');
                printWindow.document.write('<p>Tanggal Transaksi: ' + tanggalTransaksi + '</p>');
                printWindow.document.write('<p>Nama Pelanggan: ' + namaPelanggan + '</p>');
                printWindow.document.write('<p>Nama Menu: ' + namaMenu + '</p>');
                printWindow.document.write('<p>Total Harga: ' + totalHarga + '</p>');
                printWindow.document.write('<p>Jumlah Bayar: ' + jumlahBayar + '</p>');
                printWindow.document.write('<p>Kembalian: ' + kembalian + '</p>');
                printWindow.document.write('<p>Status: ' + status + '</p>');
